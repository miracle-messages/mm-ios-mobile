...
final public class YPDrawSignatureView: UIView {
    ...
    public func saveAsJPEG() -> URL? {
        if doesContainSignature {
            guard let image = getSignature() else {return nil}
            if let data = UIImageJPEGRepresentation(image, 0.8) {
                let dirPath = getDocumentDirectoryPath()
                let imageFileUrl = URL(fileURLWithPath: dirPath.appendingPathComponent("signatureImg") as String)
                do {
